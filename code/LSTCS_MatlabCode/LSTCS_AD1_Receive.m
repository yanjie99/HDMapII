% 设置MQTT客户端参数
clientID = "Cig8HicbAB8DNiMuMiM7OCE"; % AD1的ClientID
userName = "Cig8HicbAB8DNiMuMiM7OCE"; % AD1的Username
password = "NBcX3MDqTiijXBZYv+aadfRo"; % AD1的Password
...
try
    ...
    % 创建MQTT客户端并连接到broker
    mqClientAD1 = mqttclient(brokerAddress, Port = port, ClientID = clientID, Username = userName, Password = password, CARootCertificate = rootCert);

    % 检查连接状态
    if mqClientAD1.Connected
    ...
    subscribe(mqClientAD1, topicToSubscribe, 'Callback', @showmessageAD1);

    % 检查订阅状态
    subscribedTopics = mqClientAD1.Subscriptions;
...
% 回调函数定义
function showmessageAD1(~, message)
    % 使用全局变量
    global mqClientAD1;
    ...
    try
        ...
        write(mqClientAD1, feedbackTopic, feedbackMsg);
