...
% 获取actor数据
actorIDs = [1, 2, 3, 4, 5];
actorLabels = {'AD1', 'CD1', 'AD2', 'AD3', 'CD2'}; % 去掉冒号
...
for i = 1:length(actorIDs)
    ...
    for j = 1:numel(lanes)
        ...
        plot(x_coordinates, y_coordinates, 'k', 'LineWidth', 1, 'Color', [0, 0, 0, 0.2]); % 道路使用黑色调整透明度
    end
    ...
    % 设置标题和轴标签
    title(['Position of ' actorLabels{i} ' from Unknown Object Scenario'], 'FontSize', 14, 'FontWeight', 'bold');
    ...
    % 导出高分辨率图像
    print(['UOS_AgentPositionsGraph_' actorLabels{i}], '-dpng', '-r300'); % 导出为300 DPI的PNG图片
