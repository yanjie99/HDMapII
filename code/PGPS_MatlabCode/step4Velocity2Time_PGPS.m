...
plot(time, velMagAgent6, 'Color', colors(6, :), 'LineStyle', lineStyles{6}, 'LineWidth', 2);

% 设置标题和轴标签
title('Agent Velocities from Pedestrian Ghost Probe Scenario', 'FontSize', 14, 'FontWeight', 'bold');
xlabel('Time (sec)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Velocity (m/sec)', 'FontSize', 12, 'FontWeight', 'bold');

% 图例
legend({'Actor = P1', 'Actor = P2', 'Actor = AD1', 'Actor = AD2', 'Actor = CD1', 'Actor = HUM1'}, ...
...
% 导出高分辨率图像
print('AgentVelocitiesGraph', '-dpng', '-r300'); % 导出为300 DPI的PNG图片
