...
% 获取actor数据
actorIDs = [3, 4, 5, 6];
actorLabels = {'AD1', 'AD2', 'CD1', 'HUM1'}; % 去掉冒号
colors = lines(4); % 使用Matlab自带的颜色方案
...
for i = 1:length(actorIDs)
    ...
    % 设置标题和轴标签
    title(['Position of ' actorLabels{i} ' from Pedestrian Ghost Probe Scenario'], 'FontSize', 14, 'FontWeight', 'bold');
    ...
    % 导出高分辨率图像
    print(['AgentPositionsGraph_' actorLabels{i}], '-dpng', '-r300'); % 导出为300 DPI的PNG图片
